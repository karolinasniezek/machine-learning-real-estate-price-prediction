...
plt.bar(range(len(importances)), importances, color="darkgreen")

plt.xticks(
    range(len(importances)),
    [X_train.columns[i] for i in range(len(importances))],
    rotation=45
)
...
plt.savefig('../figures/feature-importances-xgboost.png')

##### Scatter Plot #####

def plot_model_vs_prediction(ax, model_name, y_true, y_pred):
    ax.scatter(y_true, y_pred, color='darkgreen', alpha=0.5)
    ax.plot([min(y_true), max(y_true)], [min(y_true), max(y_true)], color='red', linestyle='--')
    ax.set_title(f"{model_name} real vs prediction")
...
plt.savefig("../figures/scatter-models.png", dpi=300, bbox_inches="tight")
